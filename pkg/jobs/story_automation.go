...
func ProcessStoryAutoClose(ctx context.Context, db *sqlx.DB, log *logger.Logger) error {
	...
	// Use a single set-based SQL operation to close all eligible stories across all teams
	query := `
		UPDATE stories 
		SET 
			status_id = (
		...
		JOIN team_story_automation_settings tsas ON st.team_id = tsas.team_id
		WHERE stories.status_id = st.status_id
			AND st.category IN ('unstarted', 'started')
	...
	if err != nil {
		span.RecordError(err)
		log.Error(ctx, "Failed to auto-close inactive unstarted and started stories", "error", err)
		return fmt.Errorf("failed to auto-close inactive unstarted and started stories: %w", err)
	}
	...
	span.AddEvent("story auto-close completed", trace.WithAttributes(
		attribute.Int64("stories.closed", rowsAffected),
		attribute.String("duration", duration.String()),
	))

	log.Info(ctx, fmt.Sprintf("Auto-close inactive unstarted and started stories job finished: %d stories closed in %v",
		rowsAffected, duration))
