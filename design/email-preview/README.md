...
Avatar refinements: 20px circles, 8px initials, and 4px spacing before names. Name colors come directly from `packages/lib/src/avatar-color.ts`. Activity attribution is inline with the sentence; conversation replies retain a compact author line beside the message. A valid supplied `avatarURL` selects the photo; an absent or invalid URL selects name-derived initials. The photo example uses an existing landing-page sample portrait. This preview does not yet fetch live profile data. Calendar and comment icons reuse the shared frontend paths.
