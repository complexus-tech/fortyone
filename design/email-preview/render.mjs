...
function bodyContent(email, width) {
  ...
  if (email.conversation)
    result += table(
      `<tr><td style="padding:16px 0;border-top:1px solid ${rule};border-bottom:1px solid ${rule};">${label("Your comment")}${quote(email.conversation.original, "#dedad7")}</td></tr><tr><td style="padding:16px 0 0;">${email.person ? person(email.person) : `<h2 style="margin:0 0 8px;font-size:15px;line-height:21px;">${escape(email.conversation.author)}</h2>`}${quote(email.conversation.reply)}</td></tr>`,
      `style="margin-top:14px;"`,
    );
  if (email.proposal)
    result += table(
