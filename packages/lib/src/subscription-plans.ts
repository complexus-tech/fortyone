...
export type SubscriptionPlanFeatureKey = keyof SubscriptionPlan["features"];
...
} as const satisfies Partial<SubscriptionPlan["features"]>;
...
} satisfies Record<SubscriptionPlanFeatureKey, string>;

// Helper function to generate a list of feature strings from a plan
export const getSubscriptionPlanFeatures = (
  plan: SubscriptionPlan,
): string[] => {
  const features: string[] = [];

  // Add string features directly
  if (plan.features.teams) {
    features.push(
      plan.features.teams === "Unlimited"
        ? "Unlimited teams"
        : plan.features.teams,
    );
  }
  if (plan.features.objectives) {
    features.push(
      plan.features.objectives === "Unlimited"
        ? "Unlimited objectives"
        : plan.features.objectives,
    );
  }
