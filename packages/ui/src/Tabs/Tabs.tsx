...
const Trigger = ({
  ...
  return (
    <TabsPrimitive.Trigger
      value={value}
      className={cn(
        "relative w-max dark:data-[state=active]:border-dark-50 data-[state=active]:border dark:data-[state=active]:shadow dark:shadow-dark rounded-[0.6rem] border-gray-200 text-gray-300/80 dark:text-gray px-4 py-[0.2rem] hover:dark:text-gray-200 focus:dark:bg-dark-100 focus:outline-0 dark:data-[state=active]:text-gray-100 data-[state=active]:text-gray-300 data-[state=active]:bg-white data-[state=active]:dark:bg-dark-100/60 flex items-center gap-2",
        className
      )}
      {...rest}
    >
...
const List = ({ children, className, ...rest }: ListProps) => {
  return (
    <TabsPrimitive.List
      className={cn(
        "flex flex-wrap w-max mx-12 rounded-[0.6rem] bg-gray-50/50 dark:bg-dark-200/50",
        className
      )}
      {...rest}
    >
