...
const table = cva("w-full border-collapse relative", {
  variants: {
    size: {
      xs: "[&_td]:py-[0.2rem]",
      sm: "[&_td]:py-[0.4rem]",
...
const Head = ({ children, className, ...rest }: HeadProps) => {
  return (
    <thead className={cn("w-full border-b border-border", className)} {...rest}>
      {children}
    </thead>
  );
};
...
const Th = ({ children, className, ...rest }: ThProps) => {
  return (
    <th
      className={cn(
        "bg-surface-elevated z-10 min-w-[25px] py-3 sticky top-0 text-[0.88rem] font-semibold capitalize text-text-muted",
        className
      )}
      {...rest}
    >
...
const Tr = ({ children, className, ...rest }: TrProps) => {
  return (
    <tr
      className={cn(
        "border-border last:border-b-0 transition duration-200 ease-linear",
        className
      )}
      {...rest}
    >
