...
const contentClasses = cva(
  "z-50 mt-1 w-max border-[0.5px] border-border/60 bg-surface-elevated py-2 shadow-xl shadow-shadow backdrop-blur-md dark:bg-surface-elevated/80",
  {
    variants: {
      rounded: {
        sm: "rounded",
...
const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ className, ...rest }, ref) => (
    <div className="flex items-center gap-1">
      <SearchIcon
        className="relative -left-1 h-[1.15rem] w-auto text-text-muted"
        strokeWidth={2.5}
      />
      <input
        className={cn(
