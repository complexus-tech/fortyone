...
describe("POST /api/suggest-substories", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    ...
    mockedGetStory.mockResolvedValue({
      description: "Canonical parent story description",
      id: "story-1",
      subStories: [{ title: "Existing canonical substory" }],
      title: "Canonical parent story",
  ...
  it("rejects malformed, oversized, and non-locator payloads before loading context", async () => {
    const malformed = await POST(
      createRequest({
        story: { title: "Client-controlled story" },
        storyId: "story-1",
        workspaceSlug: "acme",
      }),
    );
  ...
  it("does not let guests use generation", async () => {
    ...
    const response = await POST(
      createRequest({ storyId: "story-1", workspaceSlug: "acme" }),
    );
  ...
  it("rejects a story returned outside the authorized workspace", async () => {
    mockedGetStory.mockResolvedValue({
      id: "story-1",
  ...
  it("does not reveal an inaccessible story", async () => {
    mockedGetStory.mockRejectedValue(new ApiError("Forbidden", 403, null));
