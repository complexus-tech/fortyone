...
export const listOverdue = tool({
  description:
    "List stories that are overdue (past their deadline), grouped by status. Returns stories with their details and deadlines.",
  ...
  execute: async ({ teamId, filters }) => {
    try {
      ...
      // Calculate overdue range (before today)
      ...
      return {
        ...
        meta: result.meta,
        userRole,
        message: `Found ${result.meta.totalGroups} overdue stories across ${result.groups.length} status groups.`,
      };
    } catch (error) {
      return {
        success: false,
        error: "Failed to list overdue stories",
      };
    }
  },
});
