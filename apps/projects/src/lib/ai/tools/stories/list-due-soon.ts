...
export const listDueSoon = tool({
  description:
    "List stories that are due soon (within the next 7 days), grouped by status. Returns stories with their details and deadlines.",
  ...
  execute: async ({ teamId, filters }) => {
    try {
      ...
      // Calculate due soon range (next 7 days)
      const now = new Date();
      const from = formatISO(now, { representation: "date" });
      const to = formatISO(addDays(now, 7), { representation: "date" });
      ...
      return {
        ...
        meta: result.meta,
        userRole,
        message: `Found ${result.meta.totalGroups} status groups due soon (within 7 days) across ${result.groups.length} status groups.`,
      };
    } catch (error) {
      return {
        success: false,
        error: "Failed to list stories due soon",
      };
    }
  },
});
