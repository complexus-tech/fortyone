...
export const StatusBreakdownSkeleton = () => {
  return (
    <Wrapper>
      ...
      <Box className="mb-4">
        <Box className="mx-auto h-40 w-40 animate-pulse rounded-full bg-gray-200 dark:bg-dark-50" />
      </Box>
      <Box className="flex gap-3">
        {Array.from({ length: 3 }).map((_, index) => (
          <Box className="flex items-center gap-1" key={index}>
            <Box className="h-4 w-4 animate-pulse rounded bg-gray-200 dark:bg-dark-50" />
            <Box className="h-4 w-16 animate-pulse rounded bg-gray-200 dark:bg-dark-50" />
          </Box>
        ))}
      </Box>
    </Wrapper>
