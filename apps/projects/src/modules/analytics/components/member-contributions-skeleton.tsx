      ...
      <Box className="space-y-3">
        {Array.from({ length: 6 }).map((_, index) => (
          <Flex align="center" gap={3} key={index}>
            <Box className="h-8 w-8 animate-pulse rounded-full bg-gray-200 dark:bg-dark-50" />
            <Box className="flex-1">
              <Box className="mb-2 h-4 w-24 animate-pulse rounded bg-gray-200 dark:bg-dark-50" />
              <Box className="h-2 w-full animate-pulse rounded-full bg-gray-200 dark:bg-dark-50" />
            </Box>
          </Flex>
        ))}
      </Box>
