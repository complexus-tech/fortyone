      ...
      <Box className="space-y-3">
        {Array.from({ length: 6 }).map((_, index) => (
          <Flex align="center" gap={3} key={index}>
            <Box className="bg-skeleton h-8 w-8 animate-pulse rounded-full" />
            <Box className="flex-1">
              <Box className="bg-skeleton mb-2 h-4 w-24 animate-pulse rounded" />
              <Box className="bg-skeleton h-2 w-full animate-pulse rounded-full" />
            </Box>
          </Flex>
        ))}
      </Box>
