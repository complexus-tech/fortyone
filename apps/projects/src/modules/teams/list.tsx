...
import { ObjectiveCard } from "./objective";
...
export const ObjectivesList = ({ objectives }: { objectives: Objective[] }) => {
  return (
    <>
      <ObjectivesHeader />
      <BodyContainer>
        <Container className="sticky top-0 z-[1] select-none bg-gray-50/60 py-2 backdrop-blur dark:bg-dark-300/90">
        ...
        {objectives.map(({ id, name }) => (
          <ObjectiveCard key={id} name={name} />
        ))}
      </BodyContainer>
    </>
  );
