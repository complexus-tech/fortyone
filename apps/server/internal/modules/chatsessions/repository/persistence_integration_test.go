...
func TestChatPersistenceAndMutationApprovalPostgresContract(t *testing.T) {
	...
	t.Cleanup(func() {
		_, cleanupErr := db.Exec(ctx, `DELETE FROM workspaces WHERE workspace_id IN ($1, $2)`, owner.workspaceID, other.workspaceID)
		require.NoError(t, cleanupErr)
		_, cleanupErr = db.Exec(ctx, `DELETE FROM users WHERE user_id IN ($1, $2)`, owner.userID, other.userID)
		require.NoError(t, cleanupErr)
	})
