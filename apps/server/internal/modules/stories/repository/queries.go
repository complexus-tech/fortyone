...
func (r *repo) GetStoryLinks(ctx context.Context, storyID uuid.UUID) ([]links.CoreLink, error) {
	...
	return linksrepository.ToCoreLinks(dbLinks), nil
}
									...
									json_build_object(
										...
										'story', (
											SELECT json_build_object(
												...
												'auto_scheduling_enabled', other.auto_scheduling_enabled,
												'auto_scheduling_locked', other.auto_scheduling_locked,
												'auto_scheduling_status', other.auto_scheduling_status,
												'auto_scheduling_reason', other.auto_scheduling_reason,
												'auto_scheduling_updated_at', other.auto_scheduling_updated_at,
...
func (r *repo) GetComments(ctx context.Context, storyID uuid.UUID, page, pageSize int) ([]comments.CoreComment, bool, error) {
	...
	var comments []commentsrepository.DbComment
			...
			COALESCE(
				json_agg(
					json_build_object(
						...
						'auto_scheduling_enabled', ls.auto_scheduling_enabled,
						'auto_scheduling_locked', ls.auto_scheduling_locked,
						'auto_scheduling_status', ls.auto_scheduling_status,
						'auto_scheduling_reason', ls.auto_scheduling_reason,
						'auto_scheduling_updated_at', ls.auto_scheduling_updated_at,
...
func (r *repo) mapToStoryList(storyMap map[string]any) stories.CoreStoryList {
	...
	if autoSchedulingEnabled, ok := storyMap["auto_scheduling_enabled"].(bool); ok {
		story.AutoSchedulingEnabled = autoSchedulingEnabled
	}
	if autoSchedulingLocked, ok := storyMap["auto_scheduling_locked"].(bool); ok {
		story.AutoSchedulingLocked = autoSchedulingLocked
	}
	if autoSchedulingStatus, ok := storyMap["auto_scheduling_status"].(string); ok {
		story.AutoSchedulingStatus = autoSchedulingStatus
	}
	if autoSchedulingReason, ok := storyMap["auto_scheduling_reason"].(string); ok {
		story.AutoSchedulingReason = &autoSchedulingReason
	}
	if autoSchedulingUpdatedAt, ok := parseStoryMapTime(storyMap["auto_scheduling_updated_at"]); ok {
		story.AutoSchedulingUpdatedAt = &autoSchedulingUpdatedAt
	}
				...
				SELECT
					json_agg(
						json_build_object(
							...
							'auto_scheduling_enabled', sub.auto_scheduling_enabled,
							'auto_scheduling_locked', sub.auto_scheduling_locked,
							'auto_scheduling_status', sub.auto_scheduling_status,
							'auto_scheduling_reason', sub.auto_scheduling_reason,
							'auto_scheduling_updated_at', sub.auto_scheduling_updated_at,
