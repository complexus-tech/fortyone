...
type GetNotificationEmailDeliveryParams struct {
...
func (q *Queries) GetNotificationEmailDelivery(ctx context.Context, arg GetNotificationEmailDeliveryParams) (GetNotificationEmailDeliveryRow, error) {
	row := q.db.QueryRow(ctx, getNotificationEmailDelivery, arg.NotificationID, arg.RecipientID, arg.WorkspaceID)
...
const listNotificationDeliveryTeamIDs = `-- name: ListNotificationDeliveryTeamIDs :many
...
WHERE recipient.user_id = CAST($2 AS uuid)
  AND recipient.is_active = TRUE
ORDER BY team_membership.team_id
`

type ListNotificationDeliveryTeamIDsParams struct {
	WorkspaceID uuid.UUID
	RecipientID uuid.UUID
}

func (q *Queries) ListNotificationDeliveryTeamIDs(ctx context.Context, arg ListNotificationDeliveryTeamIDsParams) ([]uuid.UUID, error) {
	rows, err := q.db.Query(ctx, listNotificationDeliveryTeamIDs, arg.WorkspaceID, arg.RecipientID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
type ListNotificationEmailDigestDeliveriesParams struct {
	RecipientID uuid.UUID
	WorkspaceID uuid.UUID
}
...
func (q *Queries) ListNotificationEmailDigestDeliveries(ctx context.Context, arg ListNotificationEmailDigestDeliveriesParams) ([]ListNotificationEmailDigestDeliveriesRow, error) {
	rows, err := q.db.Query(ctx, listNotificationEmailDigestDeliveries, arg.RecipientID, arg.WorkspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListNotificationEmailDigestDeliveriesRow{}
	for rows.Next() {
