...
func TestPaginationParamsUsesMenuDefaultPageSize(t *testing.T) {
	request := httptest.NewRequest("GET", "/labels?page=2", nil)
...
func TestToAppLabelsResponseClearsNextPageWhenComplete(t *testing.T) {
	response := toAppLabelsResponse(nil, 3, menuPageSize, false)
