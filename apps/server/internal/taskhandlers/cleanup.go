...
func NewCleanupHandlers(log *logger.Logger, db *sqlx.DB, mailerService mailer.Service, copyGenerator emailcopy.Generator, emailThreads emailthread.GuidancePreparer, systemUserID uuid.UUID, strategyNotifier jobs.StrategyNotificationCreator) *CleanupHandlers {
...
func (c *CleanupHandlers) HandleChatSessionsCleanup(ctx context.Context, t *asynq.Task) error {
	...
	if err := jobs.PurgeDeletedChatSessions(ctx, c.db, c.log); err != nil {
		c.log.Error(ctx, "Failed to purge deleted chat sessions", "error", err, "task_id", t.ResultWriter().TaskID())
		return fmt.Errorf("chat sessions cleanup failed: %w", err)
	}

	c.log.Info(ctx, "HANDLER: Successfully processed ChatSessionsCleanup task", "task_id", t.ResultWriter().TaskID())
	return nil
}
...
func (c *CleanupHandlers) HandleOverdueStoriesEmail(ctx context.Context, t *asynq.Task) error {
	...
	if err := jobs.ProcessOverdueStoriesEmail(ctx, c.db, c.log, c.mailerService, c.copyGenerator, c.emailThreads); err != nil {
...
func (c *CleanupHandlers) HandleObjectiveOverdueEmail(ctx context.Context, t *asynq.Task) error {
	...
	if err := jobs.ProcessObjectiveOverdue(ctx, c.db, c.log, c.mailerService, c.copyGenerator, c.emailThreads); err != nil {
...
func (c *CleanupHandlers) HandleWeeklyDigestEmail(ctx context.Context, t *asynq.Task) error {
	...
	if err := jobs.ProcessWeeklyDigestEmail(ctx, c.db, c.log, c.mailerService, c.copyGenerator, c.emailThreads); err != nil {
...
func (c *CleanupHandlers) HandleFeedbackDigestEmail(ctx context.Context, t *asynq.Task) error {
	...
	if err := jobs.ProcessFeedbackDigestEmail(ctx, c.db, c.log, c.mailerService, c.copyGenerator, c.emailThreads); err != nil {
