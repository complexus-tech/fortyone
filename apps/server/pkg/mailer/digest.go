...
func activityText(text string, actor EmailActor, highlights []string) template.HTML {
	...
	for _, displayName := range names {
		...
		style := fmt.Sprintf("display:inline-block;width:20px;height:20px;line-height:20px;vertical-align:middle;position:relative;top:1px;border-radius:50%%;background-color:%s;color:%s;font-family:%s;font-size:8px;font-weight:500;text-align:center;", colors[0], colors[1], emailFontStack)
