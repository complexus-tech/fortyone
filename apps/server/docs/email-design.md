...
- Public assets live in `apps/landing/public/email-assets/v1` at repository root, including the source SVG, rasterized wordmark, two illustrations, and calendar/comment/status/priority icons. Legacy licensed Inter assets remain available for already-delivered emails; new emails do not download fonts.
...
- Activity digests show each task once, using its latest event timestamp (notification ID breaks ties). Grouping happens before copy generation, counts, and the detail limit; the full batch remains covered after delivery. Task-only emails count tasks and explain that each row shows the latest update. Other entity types keep their existing behavior.
...
Activity actors are rendered inline at 20px with 8px initials and a 4px gap. Colors use the same NFKC/whitespace/lowercase normalization, UTF-16 FNV-1a hash and six-color palette as `packages/lib/src/avatar-color.ts`. Unicode parity fixtures were derived from the frontend implementation and remain in the Go tests. Copy and URLs are escaped; model output cannot inject image markup. Calendar, comment, status, and priority PNGs are packaged in the existing versioned asset folder and displayed at 24px with an 8px gap before the task title. Calendar icons cover typed dates, deadline removal, and persisted start-date changes; comment/reply events use the comment icon. Status and priority changes use their matching icons, selected from typed field metadata or the producer-owned task template. Assignment changes currently have no email icon.
...
## Scannable notification changes

Activity rows render their persisted message values directly even when the introduction is AI-generated. The task title appears once, dates/statuses/priorities/assignees are emphasized with renderer-owned `<strong>` markup, and a supplied reason appears as a separate muted paragraph. Both fallback and generated email paths use the same trusted activity row. Comment bodies and reasons are not emphasized as changed values. All text remains escaped; variable contents cannot become template placeholders or HTML.

New task status events include the previous status name when it can be resolved, so both sides of the transition are visible. Rescheduling events include the changed segment's start/end time and UTC offset; when only the start is known, only that time is shown. Initial schedule bounds can cover several work segments, so they show the start rather than implying continuous occupancy. Maya's repeated "next safe slot" explanation has been replaced by its factual reason. Already-delivered email HTML is unchanged.

Maya deadline/strategy digests and generated reminder/feedback rows emphasize their source-provided dates, deadline signals, status and progress facts. Temporary local preview galleries and their render harnesses are not application templates and were removed after verification.
