    ...
    <Container>
      ...
      >
        ...
        >
          {cards.map((card) => (
            <SwiperSlide
              className="relative rounded-lg border border-gray-100 bg-white/50 p-0.5 backdrop-blur dark:border-dark-100 dark:bg-dark-100/40 md:rounded-2xl md:p-1 dark:md:p-1.5"
              key={card.id}
            >
              <Image
                alt={card.title}
                className="relative rounded border border-gray-100 dark:border-dark-100 md:rounded-[0.8rem]"
                placeholder="blur"
                priority
