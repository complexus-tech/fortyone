...
When the "New story" dialog opens (often titled with your current team prefix, e.g., "ENG New story"):

1. **Enter title...**: Provide a clear and concise summary for the story.
2. **Story description**: Add more details, context, or acceptance criteria.
3. Optionally, set initial properties using the quick action buttons:
   - **Status** (e.g., "Backlog")
   - **Priority** (e.g., "No Priority")
   - **Start date**
   - **Deadline**
   - **Assignee**
   - **Objective**
4. If you need to create multiple stories consecutively, enable the **"Create more"** toggle.
5. Click the `+ Create story` button.
...
This area usually displays:

- **Editable Title and Description:** Directly modify the core information of the story.
- **Sub-Stories:** View, add (`+ Add Sub Issue` - note: button label may vary based on custom terminology, concept is Sub-Stories), or manage nested tasks that break down the main story.
...
- **Activity feed:** This section shows a chronological log of all changes and discussions.
  - **Comments Tab:** View and add comments to discuss the story.
  - **Updates Tab:** See a history of changes made to the story.
...
- **Status:** Change the story's current stage in the workflow (e.g., "Backlog"). (Shortcut: `S`)
- **Priority:** Adjust the story's urgency (e.g., "No Priority", "Low", "Medium", "High", "Urgent"). (Shortcut: `P`)
- **Assignee:** Reassign the story to another team member. (Shortcut: `A`)
- **Start date:** Set or update the story's start date. (Shortcut: `B` for Begin date)
- **Deadline:** Set or update the story's due date. (Shortcut: `D`)
- **Objective:** Link or change the associated objective. (Shortcut: `O`)
- **Sprint:** Assign the story to a different sprint. (Shortcut: `N` for Sprint)
- **Labels:** Add or remove labels for categorization. (Shortcut: `L`)
...
complexus aims to make you more efficient. Look out for keyboard shortcuts for common actions on stories. Besides the global `Shift + N` for new stories:
...
- `L`: Add/Manage Labels
- `D`: Set Due Date
- `B`: Set Start Date (or Begin Date)
- `O`: Link Objective
- `N`: Assign to Sprint
