...
func (r *repo) CreateWithTx(ctx context.Context, tx *sqlx.Tx, story *stories.CoreSingleStory, workspaceId uuid.UUID) (stories.CoreSingleStory, error) {
	ctx, span := web.AddSpan(ctx, "business.repository.stories.CreateWithTx")
	defer span.End()

	// Validate status belongs to the same team
	if story.Status != nil {
		if err := r.validateStatusTeamWithTx(ctx, tx, *story.Status, story.Team); err != nil {
			return stories.CoreSingleStory{}, err
		}
	}
...
func (r *repo) insertStoryWithTx(ctx context.Context, tx *sqlx.Tx, story *stories.CoreSingleStory) (dbStory, error) {
	ctx, span := web.AddSpan(ctx, "business.repository.stories.insertStoryWithTx")
	defer span.End()

	q := `
		INSERT INTO stories (
			sequence_id, title, description,
			team_id, workspace_id, status_id
		) VALUES (
			:sequence_id, :title, :description,
			:team_id, :workspace_id, :status_id
		) RETURNING 
	...
	var cs dbStory
	if err := stmt.GetContext(ctx, &cs, params); err != nil {
		errMsg := fmt.Sprintf("failed to create story: %s", err)
		r.log.Error(ctx, errMsg)
		span.RecordError(errors.New("failed to create story"), trace.WithAttributes(attribute.String("error", errMsg)))
		return dbStory{}, err
...
// validateStatusTeamWithTx validates that a status belongs to a team using an existing transaction.
func (r *repo) validateStatusTeamWithTx(ctx context.Context, tx *sqlx.Tx, statusId, teamId uuid.UUID) error {
	ctx, span := web.AddSpan(ctx, "business.repository.stories.validateStatusTeamWithTx")
	defer span.End()
