...
// TeamListWithTx retrieves states for a team using an existing transaction.
func (r *repo) TeamListWithTx(ctx context.Context, tx *sqlx.Tx, workspaceId uuid.UUID, teamId uuid.UUID) ([]states.CoreState, error) {
	ctx, span := web.AddSpan(ctx, "business.repository.states.TeamListWithTx")
	defer span.End()
