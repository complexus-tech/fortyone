...
func (r *repo) CreateSubscription(ctx context.Context, sub subscriptions.CoreWorkspaceSubscription) error {
	...
	params := map[string]any{
		...
		"stripe_subscription_item_id": sub.StripeSubscriptionItemID,
...
// UpdateSubscriptionSeatCount updates only the seat count
func (r *repo) UpdateSubscriptionSeatCount(ctx context.Context, workspaceID uuid.UUID, subID string, seatCount int) error {
	ctx, span := web.AddSpan(ctx, "business.repository.subscriptions.UpdateSubscriptionSeatCount")
	defer span.End()
	...
	result, err := stmt.ExecContext(ctx, params)
	if err != nil {
		errMsg := fmt.Sprintf("failed to update seat count: %s", err)
		r.log.Error(ctx, errMsg)
		span.RecordError(errors.New("failed to update seat count"), trace.WithAttributes(attribute.String("error", errMsg)))
		return err
	}
